  ...
  async (userId: string | number) => {
    try {
      const response = await axios.get(
        `${URL}iniciativa/getAllIniciativasAndSocials?clienteId=${userId}`
      );

      const mappedInitiatives = response.data.map(
...
export const { setFilter, setSortOrder, setSortCriteria } =
  showInitiativesSlice.actions;
