...
contract OrderbookFactory {
    ...
    function addPair(
         ...
         */
        address newOrderbook = address(new Orderbook(token1, token2));
        orderbooks[identifier] = newOrderbook;
        pairsSupported++;

        emit NewPair(token1, token2);
        emit OrderbookCreated(identifier, newOrderbook);

        return newOrderbook; // Retornar el address del Orderbook
    }
}
